...
Intelligent retrieval: Staff can ask natural language questions and receive accurate, cited answers drawn from the right documents
Role-Based Access Control (RBAC): Every retrieval query is scoped to the documents a staff member is authorised to access, enforced at the vector database level, not just the UI
...
Translate the natural language question into a SQL query using an LLM
Clean the raw LLM output to extract only the SQL statement before executing it
Execute the SQL against the database, then pass the result back to the LLM to produce a natural language answer
...
Submit the repository link on the Assignment dashboard
💡 Tips
